...
import { obtenerMetaAhorro, guardarMetaAhorro } from "../services/metaAhorroService";
...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    obtenerMetaAhorro()
      .then((meta) => {
        if (activo) setMetaAhorro(meta);
      })
      .finally(() => {
        if (activo) setCargandoMeta(false);
      });
  ...
  const montoAhorrado = Math.max(balance, 0);
  const metaPct = metaAhorro
    ? Math.min(100, Math.round((montoAhorrado / metaAhorro.montoObjetivo) * 100))
    : 0;
  const metaFaltante = metaAhorro ? Math.max(metaAhorro.montoObjetivo - montoAhorrado, 0) : 0;
  const metaAlcanzada = !!metaAhorro && montoAhorrado >= metaAhorro.montoObjetivo;

  const handleGuardarMeta = async (nombre: string, montoObjetivo: number) => {
    const guardada = await guardarMetaAhorro({ nombre, montoObjetivo });
    setMetaAhorro(guardada);
    ...
    <div className="flex-1 min-h-screen flex flex-col relative" style={{ background: `radial-gradient(ellipse 80% 60% at 12% 0%, ${C.blueSoft} 0%, transparent 55%), radial-gradient(ellipse 70% 55% at 100% 15%, ${C.turquoiseSoft} 0%, transparent 55%), radial-gradient(ellipse 60% 50% at 90% 95%, ${C.sunSoft} 0%, transparent 50%), ${C.cream}`, fontFamily: "'Space Grotesk',sans-serif" }}>
      <style>{`
        ...
        @media (min-width:1024px) {
          ...
          .area-hero{grid-area:hero;} .area-side{grid-area:side;}
      ...
      <div className="beta-grid flex-1 px-4 md:px-8 py-7 max-w-7xl w-full mx-auto relative z-10">
        ...
        <div className="area-hero relative rounded-[36px] p-7 flex flex-col justify-center gap-5 border border-white/60" style={{ background: `linear-gradient(135deg, ${C.blue}E6 0%, ${C.turquoise}CC 100%)`, backdropFilter: "blur(6px)", boxShadow: `0 1px 2px rgba(18,38,58,0.1), 0 18px 32px -14px rgba(64,95,250,0.55), 0 32px 60px -24px rgba(15,33,56,0.4)`, animation: "popIn .45s ease-out both" }}>
          ...
          {metaAhorro && !cargandoMeta && (
            <div className="absolute z-30" style={{ top: -18, right: 28, transform: "rotate(-7deg)" }}>
              <div className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full font-['Space_Grotesk'] font-extrabold text-[11px] uppercase tracking-wide" style={{ background: metaAlcanzada ? C.moss : C.sun, color: C.navy, border: `2.5px solid ${C.navy}`, boxShadow: "0 4px 0 rgba(15,33,56,0.3)" }}>
                <Sparkles size={12} /> {metaAlcanzada ? "¡Meta lograda!" : `${metaPct}% logrado`}
              </div>
            </div>
          )}

          <div className="flex items-center justify-between z-10">
            <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.9)" }}>
              <Target size={13} /> Meta de ahorro
            </div>
            {metaAhorro && (
              <button onClick={() => setModalMetaAbierto(true)} className="w-7 h-7 rounded-full flex items-center justify-center transition-transform hover:scale-110 active:scale-95" style={{ background: "rgba(255,255,255,0.2)" }} title="Editar meta">
          ...
          {cargandoMeta ? (
            <div className="flex items-center gap-2 text-[13px] py-8 justify-center z-10" style={{ color: "white" }}>
              <Loader2 size={18} className="animate-spin" /> Cargando tu meta...
            </div>
          ) : !metaAhorro ? (
            <div className="flex flex-col items-center gap-3 py-4 z-10 text-center">
              <BetoCoinHero size={88} />
              <p className="text-[13px] font-semibold max-w-xs" style={{ color: "rgba(255,255,255,0.9)", fontFamily: "'Inter',sans-serif" }}>
                Aún no tienes una meta de ahorro. ¡Define una y da seguimiento a tu progreso automáticamente!
              </p>
              <button onClick={() => setModalMetaAbierto(true)} className="flex items-center gap-1.5 px-5 py-2.5 rounded-full text-[12px] font-extrabold border-2 transition-transform hover:-translate-y-0.5 active:translate-y-0.5" style={{ background: C.sun, color: C.navy, borderColor: C.navy, boxShadow: `0 4px 0 ${C.navy}` }}>
                <Sparkles size={13} /> Establecer meta
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-6 z-10">
              <div className="relative flex-shrink-0">
                <svg width={148} height={148} className="absolute inset-0" style={{ transform: "rotate(-90deg)" }}>
                  ...
                  <circle cx={74} cy={74} r={66} fill="none" stroke={C.sun} strokeWidth={10} strokeLinecap="round"
                    ...
                    strokeDashoffset={2 * Math.PI * 66 * (1 - metaPct / 100)}
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[11px] font-bold uppercase tracking-wide mb-0.5 truncate" style={{ color: "rgba(255,255,255,0.75)", fontFamily: "'Inter',sans-serif" }}>{metaAhorro.nombre}</div>
                <div className="text-[34px] font-extrabold leading-none" style={{ color: "white" }}>
                  {formatMonto(montoAhorrado)}<span className="text-[18px] font-bold opacity-70"> / {formatMonto(metaAhorro.montoObjetivo)}</span>
                </div>
                <div className="text-[12px] mt-2 font-semibold" style={{ color: "rgba(255,255,255,0.85)", fontFamily: "'Inter',sans-serif" }}>
                  {metaAlcanzada
                    ...
                    : <>¡Vas muy bien! Faltan <strong style={{ color: C.sun }}>{formatMonto(metaFaltante)}</strong> para tu meta 🎯</>}
                </div>
                <div className="w-full rounded-full h-3 overflow-hidden mt-3" style={{ background: "rgba(255,255,255,0.25)" }}>
                  <div className="h-full rounded-full transition-[width] duration-700" style={{ width: `${metaPct}%`, background: `linear-gradient(90deg, ${C.sun}, #FFE985)` }} />
                </div>
              </div>
            </div>
          )}
        ...
        <div className="area-side">
          <div className="relative flex-1 rounded-[28px] p-6 flex flex-col border-[3px] overflow-hidden min-h-[200px]" style={{ background: "rgba(221,246,247,0.75)", backdropFilter: "blur(10px)", borderColor: C.turquoise, animation: "popIn .45s ease-out .14s both" }}>
            ...
            <div className="flex items-center gap-2.5 z-10">
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-[18px] flex-shrink-0" style={{ background: "white", boxShadow: "0 3px 0 rgba(14,116,144,0.25)" }}>💡</div>
              <div>
                <div className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#0E7490" }}>Consejo del día</div>
                <div className="text-[9.5px] font-semibold" style={{ color: "#0E7490", opacity: 0.7, fontFamily: "'Inter',sans-serif" }}>Tip {consejoIdx + 1} de {consejosDelDia.length}</div>
              </div>
            </div>

            <div className="flex-1 flex items-center z-10 py-3">
              <p className="text-[14px] leading-relaxed" style={{ fontFamily: "'Inter',sans-serif", color: C.navy }}>
            ...
            <div className="flex items-center justify-between z-10 mt-auto">
              ...
              <button onClick={() => setConsejoIdx((i) => (i + 1) % consejosDelDia.length)} className="text-[11px] font-bold px-3.5 py-1.5 rounded-full transition-transform hover:scale-105" style={{ background: C.turquoise, color: "white" }}>Más consejos →</button>
            </div>
          </div>
        </div>
      ...
      <MetaAhorroModal
        ...
        valorInicialNombre={metaAhorro?.nombre}
        valorInicialMonto={metaAhorro?.montoObjetivo}
        onGuardar={handleGuardarMeta}
        onCerrar={() => setModalMetaAbierto(false)}
      />
    </div>
  );
}
