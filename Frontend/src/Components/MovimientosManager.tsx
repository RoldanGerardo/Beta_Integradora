...
export default function MovimientosManager({ tipoVista, onNavigate }: MovimientosProps) {
  ...
  useEffect(() => {
    if (tipoVista === "balance" || !token) return;
    ...
    obtenerMovimientos(token, isIngreso ? "ingreso" : "egreso")
      .then((data) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nombre || !monto || !fecha || !token) return;
    ...
    try {
      const guardadoReal = await crearMovimiento(token, datos);
